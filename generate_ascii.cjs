...
let vLines = [];
vLines.push("┌──────────────────────────────────────────────────┐");
vLines.push("│               <span className=\"cursor-pointer hover:bg-[#c4ffff] hover:text-black transition-colors px-1 py-0.5\" onClick={handleNucleo}>" + nucleoText + "</span>                │");
vLines.push("│            " + universeNav + "            │");
vLines.push("└─────────────────────────┬────────────────────────┘");
vLines.push("                          │");
vLines.push("                          ├────────── <span className=\"cursor-pointer hover:bg-[#ff3399] hover:text-black transition-colors px-1\" onClick={handleCreative}>" + creativeText + "</span>");
vLines.push("                          │");
...
vLines.push("                          │");
vLines.push("                          └────────── <span className=\"cursor-pointer hover:bg-[#00ffcc] hover:text-black transition-colors px-1\" onClick={handleIndustry}>" + industryText + "</span>");
vLines.push("                                ");
...
export default function AsciiTree({ appMode, onSelectProject, onSetUniverse, onEnterUniverse }: AsciiTreeProps) {
  ...
  if (appMode === 'terminal') {
    return (
      <div className="font-mono text-[5.5px] sm:text-[6px] md:text-[8px] leading-tight text-[#c4ffff] opacity-90 whitespace-pre pointer-events-auto overflow-x-auto scrollbar-hide">
...
${lines.map(l => '          <span>' + l + '</span>').join('\n')}
  ...
  return (
    <div className="font-mono text-[6px] sm:text-[7px] md:text-[9px] leading-tight text-[#c4ffff] opacity-90 whitespace-pre pointer-events-auto overflow-x-auto scrollbar-hide">
      <div className="flex flex-col min-w-max pb-4">
