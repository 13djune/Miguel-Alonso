...
vLines.push("╚══════════╦══════════╝");
vLines.push("           ║");
vLines.push("           ╠══ " + "{isES ? makeClickable('[ UNIV: CREATIVO ]', 'handleCreative') : makeClickable('[ UNIV: CREATIVE ]', 'handleCreative')}");
vLines.push("           ║");
...
vLines.push("           ║");
vLines.push("           ╚══ " + "{isES ? makeClickable('[ UNIV: INDUSTRIA ]', 'handleIndustry') : makeClickable('[ UNIV: INDUSTRY  ]', 'handleIndustry')}");
vLines.push("           ");
...
// Terminal Tree (Horizontal, large)
let hLines = [];
hLines.push("                                   ╔══════════════════════════════════════════════════════════════╗");
...
export default function AsciiTree({ appMode, onSelectProject, onSetUniverse, onEnterUniverse }: AsciiTreeProps) {
  ...
  if (appMode === 'terminal') {
    return (
      <div className="font-mono text-[8px] sm:text-[10px] md:text-[15px] leading-tight text-[#c4ffff] opacity-90 whitespace-pre pointer-events-auto overflow-x-auto scrollbar-hide flex justify-center">
...
${hLines.map(l => '          <span>' + l + '</span>').join('\n')}
  ...
  return (
    <div className="font-mono text-[9px] sm:text-[10px] md:text-[11px] leading-tight text-[#c4ffff] opacity-90 whitespace-pre pointer-events-auto overflow-x-auto scrollbar-hide flex justify-center">
      <div className="flex flex-col min-w-max pb-4">
